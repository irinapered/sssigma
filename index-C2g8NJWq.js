...
if(dataParsed.eventType=='set_custom_style'){if(event.origin==='https://web.archive.org/web/20240705075319/https://web.telegram.org'){iFrameStyle.innerHTML=dataParsed.eventData;}}else if(dataParsed.eventType=='reload_iframe'){try{window.parent.postMessage(JSON.stringify({eventType:'iframe_will_reload'}),'*',);}catch(e){}
...
console.log('[Telegram.WebView] > postEvent',eventType,eventData);if(window.TelegramWebviewProxy!==undefined){TelegramWebviewProxy.postEvent(eventType,JSON.stringify(eventData));callback();}else if(window.external&&'notify'in window.external){window.external.notify(JSON.stringify({eventType:eventType,eventData:eventData}),);callback();}else if(isIframe){try{var trustedTarget='https://web.archive.org/web/20240705075319/https://web.telegram.org';trustedTarget='*';window.parent.postMessage(JSON.stringify({eventType:eventType,eventData:eventData}),trustedTarget,);callback();}catch(e){callback(e);}}else{callback({notAvailable:true});}}
...
/*
     FILE ARCHIVED ON 07:53:19 Jul 05, 2024 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 09:37:34 Aug 07, 2024.
...
playback timings (ms):
  ...
  esindex: 0.009
  cdx.remote: 7.341
  LoadShardBlock: 120.583 (3)
  PetaboxLoader3.datanode: 100.826 (4)
  load_resource: 108.386
  PetaboxLoader3.resolve: 92.882
*/